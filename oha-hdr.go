...
func main() {
	...
	case 0:
		...
		for {
			...
			responseTime, _ := strconv.ParseFloat(record[4], 64)
			h.RequestDuration = int64(1e6 * responseTime)
			hist.RecordValue(h.RequestDuration)

			offset, _ := strconv.ParseFloat(record[0], 64)
			h.RequestStart = offset
			...
			yMax = 30
			xMax = h.RequestStart + responseTime
			points = append(points, plotter.XY{X: h.RequestStart, Y: responseTime * 1000})
		}
